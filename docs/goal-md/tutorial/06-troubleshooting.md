...
# 检查网络请求
grep -n 'curl\|wget' scripts/score.sh
```
...
echo "{\"detail\":\"$(escape_json "$DETAIL")\"}"
```
...
# 统计
total_improvements = sum(d['after'] - d['before'] for d in data)
kept = sum(1 for d in data if d['result'] == 'kept')
reverted = sum(1 for d in data if d['result'] == 'reverted')

print(f"Total improvements: {total_improvements}")
print(f"Kept: {kept}, Reverted: {reverted}")

# 找出最好的行动
best = max(data, key=lambda x: x['after'] - x['before'])
print(f"Best action: {best['action']} (+{best['after'] - best['before']})")
```
...
# 例如：从 100 分扩展到 150 分
# 添加新组件
[[ -f "benches" ]] && BENCH_SCORE=$(run_benchmarks)
```
...
```markdown
| Action | Impact | How | Verified |
|--------|--------|-----|----------|
| Fix warnings | +10 | `cargo clippy --fix` | ✓ |
```
...
async function showScore() {
    const result = await exec('./scripts/score.sh');
    const score = JSON.parse(result).total;
    vscode.window.setStatusBarMessage(`GOAL: ${score}/100`);
}
```
...
```bash
# 1. 找到目标分数对应的 commit
git log --oneline --reverse iterations.jsonl
...
# 4. 创建新分支继续
git checkout -b continue-from-<score>
```
